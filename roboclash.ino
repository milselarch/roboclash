...
void loop() { 
  ...
  */
  ...
  // Serial.print("CONTROLS SWITCH ");
  if (reciever.switch1() == -1) {
    driver.normal_controls();
  } else if (reciever.switch1() == 1) {
    driver.invert_controls();
  }

  if (reciever.switch4() == 1) {
    if (reciever.switch2() == 1) {
      robot_arm->deploy_box();
    } else if (reciever.switch2() == -1) {
      robot_arm->retract_box();
    } else {
      robot_arm->neutral_box();
    }
  }
  ...
  if (pressed == 1) {
    ...
    driver.drive(0, 0);
    robot_arm->set_left_pos(80);
    robot_arm->set_right_pos(135);
  }
  ...
  */
  ...
  // driver.turn_left_motor();
  // driver.turn_right_motor();
